...
const CategoryManagement = () => {
  ...
  // Listen to menu item changes to reload categories (menuItemCount may change)
  useEffect(() => {
    const eventNames = [
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (modalType === "create") {
        ...
        await apiService.menu.updateCategory(
          selectedCategory.categoryId || selectedCategory.id,
          values
        );
        message.success("Cập nhật danh mục thành công");
        dispatchDataRefresh(DATA_REFRESH_EVENTS.CATEGORY_UPDATED, {
          ...values,
          ...
          <Card className="restaurant-card">
            <Statistic
              ...
              value={categories.reduce(
                (sum, cat) => sum + (cat.menuItemCount || 0),
                0
              )}
              prefix={<BookOutlined />}
