...
const InventoryTransactions = () => {
  ...
  const getStatistics = () => {
    ...
    const stockOutCount = transactions.filter(
      (t) => t.transactionType === "STOCK_OUT" || (t.quantity && t.quantity < 0)
    ).length;

    const adjustmentCount = transactions.filter(
      (t) =>
        t.transactionType === "ADJUSTMENT" || t.transactionType === "STOCK_TAKE"
    ).length;

    const stockTakeCount = transactions.filter(
      (t) => t.transactionType === "STOCK_TAKE"
    ).length;
      ...
      >
        <div>
          ...
          <p style={{ color: "#6b7280" }}>
            Theo dõi tất cả các giao dịch tồn kho
          </p>
        </div>
        <Space>
        ...
        <Row gutter={[16, 16]} align="middle">
          ...
          <Col xs={24} sm={4}>
            <Input
              ...
              onChange={(e) =>
                handleFilterChange("ingredientId", e.target.value)
              }
            />
          </Col>
          <Col xs={24} sm={4}>
        ...
        {selectedTransaction && (
          <div style={{ padding: "16px 0" }}>
              ...
              <Col span={12}>
                <div>
                  ...
                  <div style={{ marginLeft: "8px" }}>
                    {dayjs(selectedTransaction.transactionDate).format(
                      "DD/MM/YYYY HH:mm"
                    )}
                  </div>
                </div>
            ...
            <Row gutter={[16, 16]} style={{ marginTop: "16px" }}>
              <Col span={12}>
                <div>
                  ...
                  <div style={{ marginLeft: "8px" }}>
                    {selectedTransaction.ingredientName}
                  </div>
                </div>
              </Col>
              <Col span={12}>
            ...
            <div style={{ marginTop: "16px" }}>
              <strong>Lý do:</strong>
              <div style={{ marginLeft: "8px" }}>
                {selectedTransaction.reason || "Không có lý do"}
              </div>
            </div>
