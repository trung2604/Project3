...
const orderAPI = {
    async getAllOrders(params = {}) {
        ...
        const url = queryString ? `${API_ENDPOINTS.ORDER.BASE}?${queryString}` : API_ENDPOINTS.ORDER.BASE;
    ...
    async startCooking(orderId, updatedBy) {
        ...
        return await apiClient.post(API_ENDPOINTS.ORDER.START_COOKING(orderId), null, { params });
    },

    async markReady(orderId, updatedBy) {
        const params = updatedBy ? { updatedBy } : {};
        return await apiClient.post(API_ENDPOINTS.ORDER.MARK_READY(orderId), null, { params });
    },

    async startDelivering(orderId, updatedBy) {
        const params = updatedBy ? { updatedBy } : {};
        return await apiClient.post(API_ENDPOINTS.ORDER.START_DELIVERING(orderId), null, { params });
    },

    async completeOrder(orderId, updatedBy) {
        const params = updatedBy ? { updatedBy } : {};
        return await apiClient.post(API_ENDPOINTS.ORDER.COMPLETE(orderId), null, { params });
    }
};
