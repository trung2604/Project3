...
export const userService = {
  async login({ username, password }) {
    const res = await apiClient.post(`${USERS_BASE}/login`, {
      username,
      password,
    });
  ...
  async changeMyPassword({ currentPassword, newPassword }) {
    const res = await apiClient.patch(`${USERS_BASE}/me/password`, {
      currentPassword,
      newPassword,
    });
    return res.data?.data || res.data;
  ...
  async updateAvatar(userId, { avatarUrl, avatarPublicId }) {
    const res = await apiClient.patch(`${USERS_BASE}/${userId}/avatar`, {
      avatarUrl,
      avatarPublicId,
    });
    const data = res.data?.data || res.data;
  ...
  async updateUser(userId, payload) {
    const res = await apiClient.put(`${USERS_BASE}/${userId}`, payload);
    return res.data?.data || res.data;
  },

  async getUserByEmail(email) {
    const res = await apiClient.get(`${USERS_BASE}/email/${email}`);
    return res.data?.data || res.data;
  },

  async verifyEmail(userId) {
    const res = await apiClient.post(`${USERS_BASE}/${userId}/verify-email`);
    return res.data?.data || res.data;
  },
  ...
  async initializeRoles() {
    const res = await apiClient.post(`${USERS_BASE}/admin/initialize-roles`);
    return res.data?.data || res.data;
  },

  async getRealmRoles() {
    const res = await apiClient.get(`${USERS_BASE}/admin/realm-roles`);
    return res.data?.data || res.data;
  },

  async getClientRoles() {
    const res = await apiClient.get(`${USERS_BASE}/admin/client-roles`);
    return res.data?.data || res.data;
  },

  async syncUserRole(userId) {
    const res = await apiClient.post(`${USERS_BASE}/${userId}/sync-role`);
    return res.data?.data || res.data;
  },
};
